...
. Register your services in the `ResourceConfig` class.
...
. The `docs/schema` folder which contains type schema definitions. +
...
declares the Raml file "uses" `docs/raml/library.raml`.  This is a generated
file based on the contents of the schema library under `docs/schema`.  The
...
above example, library types would be available using `lib.MyType`
