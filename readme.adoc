...
There are 3 different forwarding containers:

* ldapforward - responsible for forwarding ldap connections to a remote ldap server
* dbforward1 - responsile for forwarding oracle connections to remote database server
* dbforward2 - responsile for forwarding oracle connections to remote database server
...
The dbforward containers work exactly the same way, but they forward oracle connections on port 1521.  You can copy/paste to make a dbforward3 if you like, changing the appropriate vars to the new number - but two are included because it is unlikely you'd need more.
